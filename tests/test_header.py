...
from ipynb_scrubber.header import HeaderOption, parse_cell_options

#: The options the tool defines, as the parser is told about them here: the
#: registry is not consulted, so a header this module reads is the one it wrote.
PARSED_OPTIONS = (
    HeaderOption('scrub-clear', takes_text=True),
    HeaderOption('scrub-omit', takes_text=False),
    HeaderOption('scrub-note', takes_text=True),
)


def options(cell_type: str, source: str) -> dict[str, Any]:
    """The option mapping a cell's header carries."""
    return parse_cell_options(cell_type, source, PARSED_OPTIONS).options


def block_styled(cell_type: str, source: str) -> frozenset[str]:
    """The names of the options a cell's header writes as a block scalar."""
    return parse_cell_options(cell_type, source, PARSED_OPTIONS).block_styled
...
def test_a_markdown_header_keeps_no_lines() -> None:
    ...
    """
    source = '<!-- scrub-clear: hello\n-->\n<!-- fig-cap: a plot -->\n## Q'
    assert parse_cell_options('markdown', source, PARSED_OPTIONS).kept == ''
